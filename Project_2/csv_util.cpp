...
int read_image_data_csv( char *filename, std::vector<char *> &filenames, std::vector<std::vector<float>> &data, int echo_file ) {
  ...
  fp = fopen(filename, "r");
  // std::cout << "Running fopen on: " << filename << std::endl;
  if( !fp ) {
    //printf("Unable to open feature file\n");
    std::cerr << "Error -> Unable to open feature file: " << filename << std::endl;
    return(-1);
  }
  ...
  for(;;) {
    ...
    if( getstring( fp, img_file ) ) {
      ...
      // std::cerr << "Error -> End of file or error reading filename: " << filename << std::endl;
      break;
    }
    // printf("Evaluting %s\n", filename);
